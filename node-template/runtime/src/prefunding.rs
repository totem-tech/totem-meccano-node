...
// This modulke functions as a pseudo-escrow module, holding funds for a specified period of time and or for a specific beneficiary.
...
// There is no automatic release of funds froim the locked state so requires that the either the deadline to have past to allow withdrawal 
// or the intervention of the third party.

// For the initial use of this prefunding module the third-party is the intended beneficiary. 
// In a later version there may be no intended third-party for example for marketplace transactions
// and therefore the funds may be locked until a cadidate secures the funds.

// A further scenario is forseen where a dispute resolution method that relies upon an independent validator to set the 
// lock release state. 
