...
pub type ReasonCodeText = Vec<u8>; // Reason for status change in text (TODO limit the size of vec)
pub type BanStatus = bool; // Ban status (default is false)

// Tuple for reason code changes
#[derive(PartialEq, Eq, Clone, Encode, Decode, Default)]
#[cfg_attr(feature = "std", derive(Debug))]
pub struct ReasonCodeStruct(ReasonCode, ReasonCodeText);
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        fn notify_project_worker(origin, worker: T::AccountId, project_hash: ProjectHashRef) -> Result {
            ...
            ensure!(<projects::Module<T>>::check_owner_valid_project(who.clone(), project_hash.clone()), "Invalid project or project owner is not correct");
           ...
           // set initial status
        //    <WorkerProjectsBacklogStatus<T>>::mutate(&status_tuple_key, |worker_projects_backlog_status| worker_projects_backlog_status.push(accepted_status));
        ...
        fn worker_acceptance_project(origin, project_hash: ProjectHashRef) -> Result {
            ...
            let status_tuple_key = (project_hash.clone(), who.clone());
                // Check that the projecxt worker has accepted the project, then update storage
                match Self::worker_projects_backlog_status(&status_tuple_key) {
                   ...
                   // the project owner will accept time bookings from the worker as long as the project is still active.
                    Some(true) => Self::store_worker_acceptance(project_hash.clone(), who.clone(), accepted_status.clone()),
                    Some(false) => (), // do nothing but should be an error TODO
                    _ => (), // do nothing but should be an error TODO
                    None => (),  // do nothing but should be an error TODO
                };
            };
        ...
        fn submit_time(
                ...
                .ok_or("This identity has not been assigned the project!")?
            {
                ...
                // set default lock and reason code default settings
                let mut text = Vec::new();
                let mut other_reason = ReasonCodeStruct(0, text.clone());
                let mut locked_reason = ReasonCodeStruct(0, text.clone());
                ...
                if let default_hash = input_time_hash {
                    ...
                    // prepare time record
                    let time_data: Timekeeper<NumberOfBlocks,LockStatus,StatusOfTimeRecord,ReasonCodeStruct,PostingPeriod,StartOrEndBlockNumber> = Timekeeper {
                    ...
                    Self::deposit_event(RawEvent::SubmitedTimeRecord(who, project_hash, time_hash));
                    ...
                    let new_time_data: Timekeeper<NumberOfBlocks,LockStatus,StatusOfTimeRecord,ReasonCodeStruct,PostingPeriod,StartOrEndBlockNumber> = Timekeeper {
                        total_blocks: number_of_blocks.into(),
                        locked_status: false,
                        locked_reason: locked_reason,
                        submit_status: 0,
...
impl<T: Trait> Module<T> {
    fn store_worker_acceptance(
        ...
        // issue event
        Self::deposit_event(RawEvent::WorkerAcceptanceStatus(who, project_hash, accepted_status));

    }

    fn store_time_record((a,b,c): (T::AccountId,ProjectHashRef,T::Hash), d: Timekeeper<NumberOfBlocks,LockStatus,StatusOfTimeRecord,ReasonCodeStruct,PostingPeriod,StartOrEndBlockNumber>) {
...
decl_event!(
    ...
    {
        SubmitedTimeRecord(AccountId, ProjectHashRef, Hash),
