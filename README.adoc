...
To achieve we are in the process of developing a custom built blockchain runtime on a peer-to-peer network, coupled with a full set of account posting recipes to create - what we believe to be  - one of the first fully decentralised products for business and consumers anywhere.


You can think of it like a globally available, dedicated public network for accounting data with a modern, intuitive, usable app front-end.

Technically speaking, Totem is delivering a fully functional accounting ledger attached to a blockchain identity. 

Having said this, it is a *_gross under-representation_* of the greater project goals which are outlined elsewhere on the interwebs.
...
Enter Totem; stage left. We think as it is 2020, we should fix this.

=== What we are working towards...

In our view, when an invoice is created at (for example) Uber, it should _automatically_ appear in your accounts. But we go one step further... this mechanism should not be dependent on a centrally controlled accounting software company, who will inevitably sell you licences (and your data) just to provide this facility. This mechanism should be open to all, completely independent of  accounting software vendors, and continue to operate, peer-to-peer, at all times, forever. 
...
brew install cmake pkg-config openssl git llvm
 ...
 - Windows:
 download Clang for Windows and install it from http://releases.llvm.org/9.0.0/LLVM-9.0.0-win64.exe[here]
 
then execute following script (PowerShell):
+
[source, shell]
----
# Install LLVM
